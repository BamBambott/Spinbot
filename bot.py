...
BOT_TOKEN = “YOUR_BOT_TOKEN_HERE”       # Replace with your token from Discord Dev Portal
SPIN_COOLDOWN_HOURS = 24                 # Hours between spins per user
WINNERS_FILE = “winners.json”           # Stores spin history

# Prize pool — adjust labels and weights freely

# Higher weight = more common. All weights are relative.
...
# Spin animation frames (cycles through these before revealing)
...
# In-memory cooldown tracker: {user_id: last_spin_datetime}
...
def spin_embed(title: str, description: str, color: discord.Color) -> discord.Embed:
...
if prize["winner"]:
    ...
    result_embed = discord.Embed(
        title="🎉 You Won!",
        description=(
            f"**{interaction.user.mention}** just spun the wheel!\n\n"
            f"**Prize: {prize['label']}**\n\n"
            f"Contact an admin to claim your reward! 🏆"
        ),
    ...
    result_embed = discord.Embed(
        ...
        description=(
            f"**{interaction.user.mention}** spun the wheel...\n\n"
            f"**{prize['label']}**\n\n"
            f"Better luck next time! Come back in **{SPIN_COOLDOWN_HOURS}h**."
        ),
...
for w in recent:
    ts = w["timestamp"][:10]
    lines.append(f"**{w['username']}** — {w['prize']} _{ts}_")
...
next_spin = "Now!" if not in_cooldown else format_cooldown(
    timedelta(hours=SPIN_COOLDOWN_HOURS) - (now - cooldowns[user_id])
)
...
if user_wins:
    last = user_wins[-1]
    embed.add_field(name="Last Prize", value=last["prize"], inline=False)
